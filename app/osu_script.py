...
def main():
    cur.execute('SELECT title FROM beatmaps_beatmap LIMIT 1')
    if not cur.fetchone():
        latest_date = get_maps(2007, 10, 6)
    else:
        cur.execute('SELECT submit_date FROM beatmaps_beatmap ORDER BY submit_date DESC LIMIT 1')
    ...
    while state:
        if latest_date == datetime.now().strftime("%Y-%m-%d"):
            ...
            latest_date = datetime(*map(int, latest_date.split("-")))
            latest_date += timedelta(days=1)
            latest_date = str(latest_date).split()[0].split("-")
            print(*latest_date)
            latest_date = get_maps(*latest_date)
...
112600 maps are downloaded in ~3.5 minutes 

but in docker speed drops a lot

Asynchronous script, in theory, would download the database in ~45 seconds, 
but due to the limitation of the OSU API, you won't know for sure.
Hes also needs a file with all the dates and hes cant upload fresh maps 
so it makes no sense to use it,
but if you are interested, you can take a look at the file osu_script_async.py
